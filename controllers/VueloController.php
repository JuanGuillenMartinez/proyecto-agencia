<?php

namespace app\controllers;

use app\models\CatAerolinea;
use app\models\Vuelo;
use yii\web\Controller;
use app\models\VueloSearch;
use yii\filters\VerbFilter;
use yii\helpers\ArrayHelper;
use app\models\CatAeropuerto;
use yii\web\NotFoundHttpException;

/**
 * VueloController implements the CRUD actions for Vuelo model.
 */
class VueloController extends Controller
{
    /**
     * @inheritDoc
     */
    public function behaviors()
    {
        return [
            'ghost-access' => [
                'class' => 'webvimark\modules\UserManagement\components\GhostAccessControl',
            ],
        ];
    }

    /**
     * Lists all Vuelo models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new VueloSearch();
        $dataProvider = $searchModel->search($this->request->queryParams);

        return $this->render('index', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
        ]);
    }

    /**
     * Displays a single Vuelo model.
     * @param int $vue_id Id
     * @return mixed
     * @throws NotFoundHttpException if the model cannot be found
     */
    public function actionView($id)
    {
        return $this->render('view', [
            'model' => $this->findModel($id),
        ]);
    }

    /**
     * Creates a new Vuelo model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate()
    {
        $model = new Vuelo();

        if ($this->request->isPost) {
            if ($model->load($this->request->post()) && $model->save()) {
                return $this->redirect(['view', 'id' => $model->vue_id]);
            }
        } else {
            $model->loadDefaultValues();
        }

        $aerolineas = ArrayHelper :: map(CatAerolinea::find()->all(), 'aer_id', 'aer_nombre');


        return $this->render('create', [
            'model' => $model,
            'aerolineas' => $aerolineas,
        ]);
    }

    /**
     * Updates an existing Vuelo model.
     * If update is successful, the browser will be redirected to the 'view' page.
     * @param int $vue_id Id
     * @return mixed
     * @throws NotFoundHttpException if the model cannot be found
     */
    public function actionUpdate($id)
    {
        $model = $this->findModel($id);

        if ($this->request->isPost && $model->load($this->request->post()) && $model->save()) {
            return $this->redirect(['view', 'id' => $model->vue_id]);
        }

        return $this->render('update', [
            'model' => $model,
        ]);
    }

    /**
     * Deletes an existing Vuelo model.
     * If deletion is successful, the browser will be redirected to the 'index' page.
     * @param int $vue_id Id
     * @return mixed
     * @throws NotFoundHttpException if the model cannot be found
     */
    public function actionDelete($id)
    {
        $this->findModel($id)->delete();

        return $this->redirect(['index']);
    }

    /**
     * Finds the Vuelo model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $vue_id Id
     * @return Vuelo the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Vuelo::findOne($id)) !== null) {
            return $model;
        }

        throw new NotFoundHttpException('The requested page does not exist.');
    }
}
